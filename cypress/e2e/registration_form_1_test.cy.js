...
describe('This is first test suite, Arina Onna', () => {
    it('User can submit data only when valid mandatory values are added', () => {
        //today's date is 3.Jul 2024
        cy.get('[data-testid="phoneNumberTestId"]').type('555666777')
        cy.get('#firstName').type('Arina')
        cy.get('#lastName').type('Onna')
        cy.get('input[name="password"]').type('Porgand27')
        cy.get('[name="confirm"]').type('Porgand27')
    ...
    */
    ...
    it('User cannot submit data when phone number is absent', () => {
        ...
        // Assert that submit button is not enabled and that successful message is not visible
            ...
            cy.get('input#phoneNumber.input').scrollIntoView()
            cy.get('input#phoneNumber.input').clear()
            cy.get('h2').contains('Password').click()
            cy.get('[name="confirm"]').type('Password123123')
            ...
            cy.get('input[data-testid="phoneNumberTestId"]').should("have.attr", "title")
